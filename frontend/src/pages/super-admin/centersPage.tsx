...
export default function CentersPage() {
  const { centers, loading, error, currentPage, lastPage, setCurrentPage } = useCenters();

  const totalCenters = centers.length;
  const activeCenters = centers.filter((center) => center.is_active).length;
  const inactiveCenters = centers.filter((center) => !center.is_active).length;
  ...
  return (
    <PageContainer>
      ...
      <div className="flex gap-6 mb-8">
        <StatCard
          icon={<img src={ercicon} alt="erc-icon" className="h-10 w-10 object-contain" />}
          value={totalCenters}
          label="Total Centers"
